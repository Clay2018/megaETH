...
import (
	...
	"log"
	"wallet/pkg/btcapi"
	extRpcClient "wallet/pkg/rpcclient"
...
type InscriptionRequest2 struct {
	CommitTxOutPointList2  []*btcapi.UnspentOutput
	CommitTxOutPointList   []*wire.OutPoint
	CommitTxPrivateKeyList []*btcec.PrivateKey // If used without RPC,
	// a local signature is required for committing the commit tx.
	// Currently, CommitTxPrivateKeyList[i] sign CommitTxOutPointList[i]
...
func NewBTCToolWithBtcApiClient(net *chaincfg.Params, request *BTCRequest) (*InscriptionTool, error) {
	if len(request.CommitTxPrivateKeyList) != len(request.CommitTxOutPointList) {
		return nil, errors.New("the length of CommitTxPrivateKeyList and CommitTxOutPointList should be the same")
	}
	tool := &InscriptionTool{
	...
	return tool, tool._initToolBTC(net, request)

}

func NewInscriptionToolWithBtcApiClient2(net *chaincfg.Params, request *InscriptionRequest2) (*InscriptionTool, error) {
	if len(request.CommitTxPrivateKeyList) != len(request.CommitTxOutPointList) {
		return nil, errors.New("the length of CommitTxPrivateKeyList and CommitTxOutPointList should be the same")
	}
	tool := &InscriptionTool{
	...
	return tool, tool._initTool2(net, request)
}

func (tool *InscriptionTool) _initToolBTC(netParams *chaincfg.Params, request *BTCRequest) error {

	totalSenderAmount := btcutil.Amount(0)
	tx := wire.NewMsgTx(wire.TxVersion)
	for i := range request.CommitTxOutPointList2 {
		txOut, err := tool.getTxOutByOutPoint2(request.CommitTxOutPointList2[i], request.CommitTxOutPointList[i])
		if err != nil {
			return err
		}
		in := wire.NewTxIn(request.CommitTxOutPointList[i], nil, nil)
		in.Sequence = defaultSequenceNum
		tx.AddTxIn(in)

		totalSenderAmount += btcutil.Amount(txOut.Value)
	}
	totalReceiveAmount := btcutil.Amount(0)
	for i := 0; i < len(request.OutPuts); i++ {
		destinationAddr, err := btcutil.DecodeAddress(request.OutPuts[i].Address, netParams)
		if err != nil {
			return err
		}
		...
		if request.OutPuts[i].Value <= 0 {
			return errors.New("invalid output.value")
		}

		totalReceiveAmount += btcutil.Amount(request.OutPuts[i].Value)
		redeemTxOut := wire.NewTxOut(request.OutPuts[i].Value, destinationAddrByte)
		tx.AddTxOut(redeemTxOut)
	}
	{
		destinationAddr, err := btcutil.DecodeAddress(request.ChangeAddress, netParams)
		if err != nil {
			return err
		}
		destinationAddrByte, err := txscript.PayToAddrScript(destinationAddr)
		...
		tx.AddTxOut(wire.NewTxOut(0, destinationAddrByte))
	}

	fee := btcutil.Amount(mempool.GetTxVirtualSize(btcutil.NewTx(tx))) * btcutil.Amount(request.FeeRate)
	changeAmount := totalSenderAmount - btcutil.Amount(totalReceiveAmount) - fee
	...
	if changeAmount > 0 {
		...
		if changeAmount < 0 {
			feeWithoutChange := btcutil.Amount(mempool.GetTxVirtualSize(btcutil.NewTx(tx))) * btcutil.Amount(request.FeeRate)

			if totalSenderAmount-btcutil.Amount(totalReceiveAmount)-feeWithoutChange < 0 {
				return errors.New("insufficient balance - 1")
			}
		}
	}
	tool.commitTx = tx
...
func (tool *InscriptionTool) _initTool2(net *chaincfg.Params, request *InscriptionRequest2) error {
	...
	err = tool.buildCommitTx2(request.CommitTxOutPointList2, request.CommitTxOutPointList, totalRevealPrevOutput, request.CommitFeeRate)
	if err != nil {
		return err
	}
...
func (tool *InscriptionTool) getTxOutByOutPoint2(outPoint2 *btcapi.UnspentOutput, outPoint *wire.OutPoint) (*wire.TxOut, error) {
	var txOut *wire.TxOut
	if outPoint2.Outpoint.Index != outPoint.Index {
		return nil, errors.New("err out point")
	}
	txOut = wire.NewTxOut(int64(outPoint2.Output.Value), outPoint2.Output.PkScript)
...
func (tool *InscriptionTool) buildCommitTx2(commitTxOutPointList2 []*btcapi.UnspentOutput, commitTxOutPointList []*wire.OutPoint, totalRevealPrevOutput, commitFeeRate int64) error {
	totalSenderAmount := btcutil.Amount(0)
	tx := wire.NewMsgTx(wire.TxVersion)
	var changePkScript *[]byte
	for i := range commitTxOutPointList {
		txOut, err := tool.getTxOutByOutPoint2(commitTxOutPointList2[i], commitTxOutPointList[i])
		if err != nil {
			return err
		}
	...
	fee := btcutil.Amount(mempool.GetTxVirtualSize(btcutil.NewTx(tx))+int64(34*len(tool.txCtxDataList))) * btcutil.Amount(commitFeeRate)
	...
	if changeAmount > 0 {
		...
		if changeAmount < 0 {
			...
			if totalSenderAmount-btcutil.Amount(totalRevealPrevOutput)-feeWithoutChange < 0 {
				return errors.New("insufficient balance - 2")
			}
		}
	}
	tool.commitTx = tx
...
func (tool *InscriptionTool) buildCommitTx(commitTxOutPointList []*wire.OutPoint, totalRevealPrevOutput, commitFeeRate int64) error {
	...
	if changeAmount > 0 {
		...
		if changeAmount < 0 {
			feeWithoutChange := btcutil.Amount(mempool.GetTxVirtualSize(btcutil.NewTx(tx))) * btcutil.Amount(commitFeeRate)
			if totalSenderAmount-btcutil.Amount(totalRevealPrevOutput)-feeWithoutChange < 0 {
				return errors.New("insufficient balance - 3")
			}
		}
	}
	tool.commitTx = tx
