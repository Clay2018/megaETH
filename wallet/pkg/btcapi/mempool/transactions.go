...
func (c *MempoolClient) GetTransaction(txHash string) (*wire.MsgTx, error) {
	res, err := c.request(http.MethodGet, fmt.Sprintf("/tx/%s/raw", txHash), nil)
	if err != nil {
		return nil, err
	}
...
func (c *MempoolClient) BroadcastTx2(rawTransaction string) (*chainhash.Hash, error) {

	res, err := c.request(http.MethodPost, "/tx", strings.NewReader(rawTransaction))
	if err != nil {
		return nil, err
	}
